...
/* The top of the first surface under a spot, so level data can be written by
   eye and everything still lands on something. Snapped to the terrain grid,
   because that is where the surface actually is once it is drawn — otherwise
   a gate or a parked tank sits a few pixels into the floor. */
function ground(w, x, wide, y) {
  ...
  return Math.floor(top / S.CELL) * S.CELL;
}
...
function build(entry, opt) {
  ...
  /* The floor gives — dig into the scrapyard all you like — but the authored
     ledges hold. A level is a set of jumps somebody measured; a slam through
...
function view() {
  ...
  return {
    view: {
      ...
      grid: w.grid, bg: w.bg, players: w.players, enemies: w.enemies,
